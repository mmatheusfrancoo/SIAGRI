...
uses
  ...
  Web.WebBroker,
  Datasnap.DSSession,
  ServerMethodsUnit in 'ServerMethodsUnit.pas' {SM: TDSServerModule},
  WebModuleUnit in 'WebModuleUnit.pas' {WebModule1: TWebModule},
  ServerConst in 'ServerConst.pas',
  UJson in 'unt\Utilidades\UJson.pas',
  untDMConexao in 'untDMConexao.pas' {DM: TDataModule},
  UUtilities in 'unt\Utilidades\UUtilities.pas';

{$R *.res}

function RetotnaPortaHTTP: Integer;
begin
  Result  := 8080;
end;
...
begin
  ...
  // Esta Procedure vai almentar a quantidade de caracter disponivel na basse URB.
  WebModule := TWebModule1.Create(nil);
  try
    LServer.OnContextCreated := WebModule.OnContextCreated;
  finally
    FreeAndNil(WebModule);
  end;
